        ...
        {favourites.length === 0 ? (
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto">
            {favourites.map((fav) => (
              <div
                key={fav.id}
                className="flex gap-3 bg-slate-50 p-2 rounded-lg group relative"
              >
                <img
                  src={fav.images[0]}
                  alt="thumb"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-serif text-sm text-slate-800 truncate leading-tight">
                    {fav.description}
                  </p>
                  <Link
                    to={`/property/${fav.id}`}
                    className="text-xs font-semibold tracking-wide uppercase text-blue-600 hover:underline mt-1 block"
                  >
                    View
                  </Link>
                </div>
